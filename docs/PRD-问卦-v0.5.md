...
**天问 v0.5** 是体验打磨版本，新增三个轻量功能：卦象分享卡片（一键生成图片分享到社交平台）、掷爻音效（铜钱落地的沉浸感）、铜钱 3D 抛掷动画（视觉仪式感提升）。不增加新的起卦算法，全是对现有体验的增强。
